...
async def test_graph_metadata_round_trip(adapter):
...
async def test_attach_node_source_refs_materializes_all_fields(adapter):
    d1, d2 = uuid4(), uuid4()
    r1, r2 = uuid4(), uuid4()
...
async def test_attach_without_pipeline_run_is_not_rollbackable_by_run(adapter):
    d1, r1 = uuid4(), uuid4()
...
async def test_remove_node_source_refs_updates_derived_and_is_idempotent(adapter):
...
async def test_node_source_ref_lookups_are_exact(adapter):
    d1, r1 = uuid4(), uuid4()
    key_a = make_source_ref_key(d1, uuid4())
...
async def test_remove_keeps_dataset_id_when_sibling_ref_shares_dataset(adapter):
...
async def test_remove_keeps_run_id_when_sibling_ref_shares_run(adapter):
...
async def test_node_delete_data_snapshot_fields(adapter):
    germany = _Ent(id=uuid4(), name="Germany")
...
async def test_edge_provenance_snapshot_and_lookups(adapter):
    d1, r1 = uuid4(), uuid4()
    key = make_source_ref_key(d1, uuid4())
    germany_id, alice_id = await _seed_two_entities(adapter)
...
async def test_delete_edge_triples_preserves_endpoints(adapter):
    germany_id, alice_id = await _seed_two_entities(adapter)
    await adapter.add_edges([(germany_id, alice_id, "knows", {"edge_text": "t"})])
...
async def test_remove_belongs_to_set_tags_scoped_and_unscoped(adapter):
...
async def test_rewrite_preserves_provenance(adapter):
    d1, r1 = uuid4(), uuid4()
    key = make_source_ref_key(d1, uuid4())
    germany = _Ent(id=uuid4(), name="Germany")
    alice = _Ent(id=uuid4(), name="Alice")
...
async def test_add_nodes_folds_provenance_in_one_write(adapter):
    d1, r1 = uuid4(), uuid4()
    key = make_source_ref_key(d1, uuid4())
...
async def test_add_edges_folds_provenance_in_one_write(adapter):
    d1, r1 = uuid4(), uuid4()
    key = make_source_ref_key(d1, uuid4())
    germany_id, alice_id = await _seed_two_entities(adapter)
...
async def test_add_edges_folds_multiple_owners(adapter):
    d1, d2, r1, r2 = uuid4(), uuid4(), uuid4(), uuid4()
    key_a = make_source_ref_key(d1, uuid4())
    key_b = make_source_ref_key(d2, uuid4())
    germany_id, alice_id = await _seed_two_entities(adapter)
...
async def test_folded_attach_omitted_when_no_source_ref(adapter):
    node = _Ent(id=uuid4(), name="Germany")
...
async def test_folded_attach_preserves_model_a_on_node_and_edge_reattach(adapter):
...
async def test_concurrent_folded_attach_keeps_all_keys(adapter):
    ds = uuid4()
    run = str(uuid4())
...
async def test_concurrent_explicit_attach_keeps_all_keys(adapter):
    ds = uuid4()
    run = str(uuid4())
    key1 = make_source_ref_key(ds, uuid4())
    key2 = make_source_ref_key(ds, uuid4())
